...
class AdvancedProblemRecognizer {
 public:
  struct Pattern {
    ...
    Pattern(int vars, Objective obj, const std::set<std::string> &constraints, const std::set<std::string> &recurrences,
            ProblemType t)
        : num_state_vars(vars),
          objective(obj),
  ...
  static ProblemType recognize(const DPProblem &problem) {
    ...
    for (const auto &pattern : patterns) {
      if (pattern.num_state_vars == num_vars && pattern.objective == obj &&
 ...
 private:
  ...
  static void registerPatterns() {
    // LIS pattern
    patterns.emplace_back(1,  // Single state variable
                          Objective::MAXIMIZE, {"monotonic_increase", "order_constraint"},
                          {"max_previous", "subsequence_relation"}, ProblemType::LIS);

    // LCS pattern
    patterns.emplace_back(2,  // Two state variables
                          Objective::MAXIMIZE, {"sequence_comparison", "2d_constraints"}, {"diagonal_match", "2d_max"},
                          ProblemType::LCS);

    // Convex GLWS pattern
    patterns.emplace_back(1,  // Single or double state variable
                          Objective::MINIMIZE, {"interval_constraint", "convex_property"},
                          {"min_cost", "interval_recurrence"}, ProblemType::CONVEX_GLWS);
  }
  ...
  static bool hasMatchingConstraints(const std::set<std::string> &actual, const std::set<std::string> &pattern) {
...
class ExtendedDPProblem : public DPProblem {
 public:
  ...
  void addRecurrenceHint(const std::string &hint) { recurrence_hints.push_back(hint); }

  void addProblemDescription(const std::string &description) { problem_description = description; }

  // Get metadata
  const std::vector<std::string> &getRecurrenceHints() const { return recurrence_hints; }

  const std::string &getProblemDescription() const { return problem_description; }
...
class DSLCompiler {
 ...
 private:
  static std::string generateIR(const DPProblem &problem) {
    ...
    ir << "OBJECTIVE: " << (problem.getObjective() == Objective::MAXIMIZE ? "MAX" : "MIN") << "\n";
    ...
    ir << "CONSTRAINTS:\n";
    for (const auto &constraint : problem.getConstraints()) {
      ir << "  " << constraint.var1 << " " << constraintTypeToString(constraint.type) << " " << constraint.var2 << "\n";
    }
  ...
  static std::string constraintTypeToString(Constraint::ConstraintType type) {
    switch (type) {
      case Constraint::ConstraintType::LESS_THAN:
        return "<";
      case Constraint::ConstraintType::GREATER_THAN:
        return ">";
      case Constraint::ConstraintType::EQUAL:
        return "==";
      case Constraint::ConstraintType::NOT_EQUAL:
        return "!=";
      default:
        return "??";
    }
