...
private:
    ...
    struct alignas(128) Node {  // Cacheline alignment to prevent false sharing
        ...
        Node(size_t idx, size_t left_idx, size_t right_idx, size_t parent_idx) 
            : needs_update(false), index(idx), left_child_index(left_idx), 
              right_child_index(right_idx), parent_index(parent_idx), is_leaf(false) 
        {
        ...
        Node(size_t idx, size_t parent_idx, const T& value) 
            : needs_update(false), index(idx), left_child_index(SIZE_MAX), 
              right_child_index(SIZE_MAX), parent_index(parent_idx), is_leaf(true) 
        {
...
public:
     ...
     */
    LockFreeTournamentTree(size_t capacity, T sentinel = std::numeric_limits<T>::max(), 
    ...
    bool insert(size_t index, const T& value) {
        ...
        typename Node::VersionedValue current = 
            leaf_node->versioned_value.load(std::memory_order_acquire);
        ...
        bool success = false;
        do {
            success = leaf_node->versioned_value.compare_exchange_strong(
                current, new_val, std::memory_order_acq_rel, std::memory_order_acquire);
     ...
     */
    T get_winner() const {
        typename Node::VersionedValue root_val = 
            nodes[0]->versioned_value.load(std::memory_order_acquire);
    ...
    T extract_winner() {
        ...
        typename Node::VersionedValue current = 
            winner_leaf->versioned_value.load(std::memory_order_acquire);
        ...
        bool success = false;
        do {
            success = winner_leaf->versioned_value.compare_exchange_strong(
                current, new_val, std::memory_order_acq_rel, std::memory_order_acquire);
...
private:
    ...
    bool update_node(size_t idx) {
        ...
        typename Node::VersionedValue left_val = 
            nodes[left_idx]->versioned_value.load(std::memory_order_acquire);
        typename Node::VersionedValue right_val = 
            nodes[right_idx]->versioned_value.load(std::memory_order_acquire);
        ...
        typename Node::VersionedValue current = 
            node->versioned_value.load(std::memory_order_acquire);
        ...
        bool success = false;
        do {
            success = node->versioned_value.compare_exchange_strong(
                current, new_val, std::memory_order_acq_rel, std::memory_order_acquire);
